...
const buildApiKeyContextRows = (
  ...
  fallback?: MonitoringAnalyticsApiKeyStatRow
    ...
    rows && rows.length > 0
      ...
      : fallback
        ? [
            {
              id:
                fallback.source_hashes?.[0] ||
                fallback.auth_indices?.[0] ||
                fallback.api_key_hash ||
                fallback.id ||
                '-',
              auth_provider_snapshot: fallback.auth_provider_snapshot,
              account_snapshot: fallback.account_snapshot,
              auth_label_snapshot: fallback.auth_label_snapshot,
              auth_index: fallback.auth_indices?.[0],
              source: fallback.sources?.[0],
              source_hash: fallback.source_hashes?.[0],
              calls: fallback.calls,
              success_calls: fallback.success_calls,
              failure_calls: fallback.failure_calls,
              success_rate: fallback.success_rate,
              failure_rate: safeShare(toNumber(fallback.failure_calls), toNumber(fallback.calls)),
              total_tokens: fallback.total_tokens,
              cost: fallback.cost,
              average_latency_ms: fallback.average_latency_ms,
              last_seen_ms: fallback.last_seen_ms,
            },
          ]
        : [];
        ...
        share:
          totalCost > 0
            ...
            : totalTokens > 0
              ? toNumber(row.total_tokens) / totalTokens
              : 0,
        contexts: buildApiKeyContextRows(row.contexts, row),
