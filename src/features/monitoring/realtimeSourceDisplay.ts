...
export const buildRealtimeSourceDisplay = (
  ...
  // When a channel carries a protocol suffix (e.g. 'opencode-go·claude'),
  // prefer showing "account·protocol" (e.g. 'user@example.com·claude')
  // so individual keys are distinguishable in the SOURCE column.
  const channelProtocolSuffix =
    channel && channel.includes('·') ? channel.slice(channel.indexOf('·')) : '';
  const accountWithProtocol =
    channelProtocolSuffix && account ? `${account}${channelProtocolSuffix}` : '';
  const primary =
    firstReadable(
      accountWithProtocol,
