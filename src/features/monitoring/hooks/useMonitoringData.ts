...
import { buildFilterOptionsFromSelector } from '../model/selectorAdapters';
...
export const buildMonitoringAnalyticsInclude = ({
  eventsBeforeMs,
  eventsBeforeId,
  granularity,
}: {
  eventsBeforeMs: number | null;
  eventsBeforeId: number | null;
...
export const buildMonitoringSelectorInclude = ({
  granularity,
}: {
  granularity: 'hour' | 'day';
}): MonitoringAnalyticsInclude => ({
...
export const resolveMonitoringEventsNextCursor = (
  page:
    | Pick<MonitoringAnalyticsEventsResponse, 'next_before_ms' | 'next_before_id'>
    | null
    | undefined
) => {
  const nextBeforeMs = page?.next_before_ms;
  if (!nextBeforeMs) return null;
  return {
    beforeMs: nextBeforeMs,
    beforeId: page.next_before_id ?? null,
  };
};

export const resolveMonitoringSelectorFilterOptions = <T,>({
  selectorFilterOptions,
  lastSelectorFilterOptions,
  fallbackFilterOptions,
  buildOptionsFromSelector,
}: {
  selectorFilterOptions: MonitoringAnalyticsFilterOptions | null | undefined;
  lastSelectorFilterOptions: MonitoringAnalyticsFilterOptions | null | undefined;
  fallbackFilterOptions: T;
  buildOptionsFromSelector: (options: MonitoringAnalyticsFilterOptions) => T;
}): T => {
  const options = selectorFilterOptions ?? lastSelectorFilterOptions;
  return options ? buildOptionsFromSelector(options) : fallbackFilterOptions;
};
...
export function useMonitoringData({
  ...
  const [lastSelectorFilterOptions, setLastSelectorFilterOptions] =
    useState<MonitoringAnalyticsFilterOptions | null>(null);
  ...
  const analytics = useMonitoringAnalytics({
    ...
    include: buildMonitoringAnalyticsInclude({
      eventsBeforeMs,
      eventsBeforeId,
  ...
  const selectorAnalytics = useMonitoringAnalytics({
    fromMs: analyticsBounds?.startMs,
    toMs: analyticsBounds?.endMs,
    nowMs: analyticsNowMs,
    dataScopeKey: `${eventsScopeKey}:selectors`,
    searchQuery,
    searchApiKeyHash,
    filters: analyticsFilters,
    include: buildMonitoringSelectorInclude({ granularity: analyticsGranularity }),
    throttleMs: 1_000,
  });
  const analyticsData = analytics.data;
  const currentAnalyticsData = analytics.dataStale ? null : analyticsData;
  const selectorAnalyticsData = selectorAnalytics.dataStale ? null : selectorAnalytics.data;

  useEffect(() => {
    const selectorFilterOptions = selectorAnalyticsData?.filter_options;
    if (selectorFilterOptions) {
      setLastSelectorFilterOptions(selectorFilterOptions);
    }
  }, [selectorAnalyticsData?.filter_options]);
  ...
  const loadMoreEvents = useCallback(() => {
    if (
      ...
      return;
    const nextCursor = resolveMonitoringEventsNextCursor(currentAnalyticsData?.events);
  ...
  const filterOptions = useMemo(() => {
    const base = resolveMonitoringSelectorFilterOptions({
      selectorFilterOptions: selectorAnalyticsData?.filter_options,
      lastSelectorFilterOptions,
      fallbackFilterOptions,
      buildOptionsFromSelector: (options) =>
        buildFilterOptionsFromSelector(
          options,
          authMetaMap,
          authFileMap,
          sourceInfoMap,
    ...
    // Supplement analytics channels with event-row channels that have
    // executor_type-based sub-channels (e.g. opencode-go·claude).
    // When sub-channels exist, replace the parent channel to avoid
    // a stale parent entry that matches no events.
    const eventChannels = uniqueOptionValues(
      rangeFilteredRows.map((row) => row.channel)
    );
    if (eventChannels.length === 0) return base;
    const subChannelsByParent = new Map<string, string[]>();
    for (const ch of eventChannels) {
      const sepIdx = ch.indexOf('·');
      if (sepIdx > 0) {
        const parent = ch.slice(0, sepIdx);
        const arr = subChannelsByParent.get(parent) || [];
        if (!arr.includes(ch)) arr.push(ch);
        subChannelsByParent.set(parent, arr);
      }
    }
    if (subChannelsByParent.size === 0) return base;
    const merged: string[] = [];
    for (const ch of base.channels) {
      const subs = subChannelsByParent.get(ch);
      if (subs) {
    ...
    // Also add any event-only sub-channels not covered by analytics channels
    for (const ch of eventChannels) {
      if (!merged.includes(ch)) merged.push(ch);
    }
    const channelsChanged =
      merged.length !== base.channels.length ||
      merged.some((ch, i) => ch !== base.channels[i]);
    return channelsChanged ? { ...base, channels: merged } : base;
  }, [
    apiKeyDisplayMap,
    authFileMap,
    authMetaMap,
