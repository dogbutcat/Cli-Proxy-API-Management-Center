...
import { formatCompactNumber, formatUsd, getTotalCachedTokens } from '@/utils/usage';
...
const buildApiKeySecondaryText = (row: MonitoringApiKeyRow) => {
  ...
  if (row.apiKeyHash) {
    return formatApiKeyHashLabel(row.apiKeyHash);
  }
  return '';
};
...
const buildApiKeySummaryMetrics = (
  ...
  },
  {
    ...
    value: formatCompactNumber(
      getTotalCachedTokens(row.cachedTokens, row.cacheReadTokens, row.cacheCreationTokens)
    ),
  },
  {
...
function ApiKeySummaryPrimary({
  ...
  const keyLabel = row.isUnknown
    ? row.channels.find((ch) => ch && ch !== '-') || t('monitoring.api_key_unknown_label')
    : row.apiKeyLabel || row.apiKeyMasked || t('monitoring.api_key_unknown_label');
  const firstSourceRaw = (row.sourceLabels?.[0] || '').replace(/^[a-z]:/, '');
  const copyText = row.rawApiKey || row.apiKeyMasked || row.apiKeyHash || firstSourceRaw || '';
