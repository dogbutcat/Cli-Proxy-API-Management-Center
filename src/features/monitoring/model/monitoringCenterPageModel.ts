...
const buildApiKeyOptionsFromMap = (
  ...
  ensureSelectedOption(
    ...
    selectedApiKeyHash,
    formatApiKeyHashLabel(selectedApiKeyHash)
  );

const getApiKeyOptionLabel = ({
  apiKeyHash,
  apiKeyLabel,
  apiKeyMasked,
}: {
  ...
  apiKeyLabel?: string;
  apiKeyMasked?: string;
}) => apiKeyLabel || apiKeyMasked || formatApiKeyHashLabel(apiKeyHash);
...
export const buildApiKeyOptions = (
  ...
  rows.forEach((row) => {
    ...
    optionMap.set(row.apiKeyHash, getApiKeyOptionLabel(row));
  });
...
const formatResetDuration = (seconds: number, t: TFunction): string => {
  if (seconds <= 0) return '-';
  const d = Math.floor(seconds / 86400);
  const h = Math.floor((seconds % 86400) / 3600);
  const m = Math.floor((seconds % 3600) / 60);
  let duration: string;
  if (d > 0) duration = `${d}d ${h}h`;
  else if (h > 0) duration = `${h}h ${m}m`;
  else duration = `${m}m`;
  return t('opencode_go_quota.reset_duration', { duration });
};

const buildOpenCodeGoAccountQuotaWindows = (
  quota: OpenCodeGoQuota,
  t: TFunction
): AccountQuotaWindow[] => {
  const windowDefs: { key: 'rolling' | 'weekly' | 'monthly'; labelKey: string }[] = [
  ...
  return windowDefs
    .filter((def) => quota[def.key] != null)
    .map((def) => {
      const w = quota[def.key]!;
      return {
        id: def.key,
        label: t(def.labelKey),
        remainingPercent: clampRemainingPercent(w.percentRemaining),
        resetLabel: formatResetDuration(w.resetInSec, t),
        usageLabel: null,
      };
    });
};
export const getAccountQuotaProviderLabel = (
  ...
  switch (provider) {
    ...
    case 'opencode-go':
      return t('opencode_go_quota.title');
    case 'xai':
...
const getAccountQuotaEmptyMessage = (provider: MonitoringAccountQuotaProvider, t: TFunction) => {
  switch (provider) {
    ...
    case 'opencode-go':
      return t('opencode_go_quota.empty_windows');
    case 'xai':
...
const readOpenCodeGoQuotaEntryName = (file: AuthFileItem): string => {
  const candidates = [
...
export const requestAccountQuota = async (
  ...
  switch (target.provider) {
    ...
    case 'opencode-go': {
      const entryName = readOpenCodeGoQuotaEntryName(target.file);
      if (!entryName) {
        return stampAccountQuotaFetchTime({
          ...buildBaseAccountQuotaEntry(target, t),
          windows: [],
          error: t('opencode_go_quota.missing_entry_name', {
            defaultValue: 'Missing OpenCode Go quota entry name',
          }),
        });
      }

      const result = await opencodeGoApi.refreshQuotaByEntryName(entryName);
      if (result.error) {
        return stampAccountQuotaFetchTime({
          ...buildBaseAccountQuotaEntry(target, t),
          windows: [],
      ...
      return stampAccountQuotaFetchTime({
        ...buildBaseAccountQuotaEntry(target, t),
        windows: result.quota
          ? buildOpenCodeGoAccountQuotaWindows(result.quota, t)
          : [],
      });
    }
    case 'codex':
