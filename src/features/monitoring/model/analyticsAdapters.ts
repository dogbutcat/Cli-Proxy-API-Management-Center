...
/**
 * Normalize a provider snapshot value to its canonical type.
 * Backend stores executor keys like 'openai-compatible-opencode-go' — strip
 * the instance suffix so the Providers filter shows the generic 'openai-compatible'.
 */
const normalizeProviderType = (value: string | null | undefined): string => {
  const trimmed = readString(value).trim().toLowerCase();
  if (trimmed.startsWith(OPENAI_COMPAT_PREFIX)) return 'openai-compatible';
  return trimmed;
};
...
export const buildAnalyticsFilters = (
  ...
  if (isActiveFilterValue(scopeFilters.provider)) {
    ...
    const isOpenAICompatFilter = normalizedProvider === 'openai-compatible';
    const providerAuthIndices = Array.from(authMetaMap.entries())
      .filter(([, meta]) => {
        const metaProvider = normalizeFilterText(meta.provider);
        return isOpenAICompatFilter
          ? metaProvider.startsWith('openai-compatible')
          : metaProvider === normalizedProvider;
      })
      .map(([authIndex]) => authIndex);
  ...
  if (isActiveFilterValue(scopeFilters.channel)) {
    ...
    if (channelAuthIndices.length === 0) {
      // Sub-channel with '·' separator (e.g. 'opencode-go·claude') carries
      // executor protocol info. Split into provider + request_types so the
      // backend can filter by both provider and executor_type.
      const sepIdx = channel.indexOf('·');
      if (sepIdx > 0) {
        const parentProvider = channel.slice(0, sepIdx);
        const protocol = channel.slice(sepIdx + 1);
        if (!filters.providers?.includes(parentProvider)) {
          filters.providers = [...(filters.providers || []), parentProvider];
        }
        // Map user-friendly protocol names to executor type class names.
        const executorType =
          protocol === 'claude'
            ? 'ClaudeExecutor'
            : protocol === 'openai'
              ...
              : protocol;
        filters.request_types = [...(filters.request_types || []), executorType];
      } else if (!filters.providers?.includes(channel)) {
...
export const buildAccountRowsFromAnalytics = (
  ...
  rows
    .map((row) => {
      ...
      const channels = uniqueReadableValues([display.channel, ...channelNames]);
      ...
      const uniqueId = account && account !== row.id ? `${account}::${row.id}` : row.id;
      return {
        id: uniqueId,
        account,
        displayAccount,
  ...
  rows
    .map((row) => {
      ...
      return {
        ...
        rawApiKey: apiKeyDisplay?.rawKey || display.rawApiKey,
        isUnknown,
        ...
        channels: uniqueReadableValues([display.channel, ...channelNames]),
        totalCalls: row.calls,
        successCalls: row.success_calls,
        failureCalls: row.failure_calls,
        successRate: row.success_rate,
    ...
    channels: uniqueReadableValues(
      channelRows.map((row) => {
        ...
        return (
          ...
          sourceInfoMap.byAuthIndex.get(authIndex)?.providerName ||
...
export const buildUsageDetailsFromAnalyticsEvents = (
  ...
  items.map((item) => ({
    ...
    api_key_hash:
      readString(item.api_key_hash) || readString(item.source_hash) || readString(item.source),
