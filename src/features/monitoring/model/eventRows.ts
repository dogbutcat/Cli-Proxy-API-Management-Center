...
/**
 * Map executor type names to user-friendly protocol suffixes for channel display.
 * Returns undefined when no suffix should be appended.
 */
const executorProtocolSuffix = (executorType: string): string | undefined => {
  const lower = executorType.toLowerCase();
  if (lower.includes('claude')) return 'claude';
  if (lower.includes('openaicompat') || lower.includes('openai')) return 'openai';
  return undefined;
};
...
export const buildEventRows = (
  ...
  details
    .map((detail, index) => {
      ...
      const protocolSuffix = executorType ? executorProtocolSuffix(executorType) : undefined;
      const channelLabel =
        protocolSuffix && baseChannel.toLowerCase() === 'opencode-go'
          ? `${baseChannel}·${protocolSuffix}`
          : baseChannel;
