...
export const resolveMonitoringAccountQuotaProvider = (
  ...
  if (isOpenCodeGoFile(file)) return 'opencode-go';
  return null;
};
...
const resolveActiveQuotaProvidersForRow = (
  ...
  // When a single account row spans multiple providers (e.g. same email used for
  // both codex and opencode-go), restrict quota targets to only the providers
  // that actually had usage events in this row, identified by the row's channels.
  if (activeProviders.size > 1 && row.channels.length > 0) {
    const channelSet = new Set(row.channels.map((ch) => ch.toLowerCase()));
    for (const provider of activeProviders) {
      if (!channelSet.has(provider)) {
