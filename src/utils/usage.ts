...
export function calculateCost(
  ...
  const standardCost =
    cacheReadTokens > 0 || cacheCreationTokens > 0
      ...
      : promptCost + cachedCost + completionCost;
...
/**
 * Compute the total cache tokens from the three independent cache dimensions.
 *
 * Backend's `CompatibleCachedTokens` subtracts fine-grained cache fields from
 * the legacy `cached_tokens` to prevent double-counting, so the rollup/API
 * value of `cachedTokens` is the *residual* (often 0 for Claude-protocol data).
 * Adding the three fields back together recovers the true total:
 *
 *   Total = residual(cachedTokens) + cacheReadTokens + cacheCreationTokens
 *
 * This is safe for all data shapes:
 * - Claude data: residual=0, read+creation=full → correct
 * - OpenAI data: residual=full, read=0, creation=0 → correct
 * - Legacy data: residual=N, read=0, creation=0 → correct
 */
export function getTotalCachedTokens(
  cachedTokens: number,
